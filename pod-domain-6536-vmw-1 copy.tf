...
module "intersight_pod1_domain_2" {                                             # <-- change when copying domain
  ...
  policy_prefix = "${local.pod_policy_prefix}-domain-2" # <-- change when copying domain
  ...
  tags = [
    ...
    { "key" : "domain", "value" : "${local.pod_policy_prefix}-domain-2" } # <-- change when copying domain
  ]
